...
| **前瞻记录 ★** | `forward`:把风控后组合接成**只前进、不回看**的前瞻纸面曲线(每天追加真实净值点,历史绝不改写)——用真实流逝的时间兑现"回测会骗人" |
| **界面** | Flask 网页台(8 页,含**前瞻曲线**页)+ 16 个 CLI 命令 + 双击启动 |
...
└── watchlist.py     自选股加载
tests/               144 个离线测试(含「无未来函数」「只前进不回看」锁)
...
```bash
./.venv/bin/python -m pytest -q        # 144 个离线测试
